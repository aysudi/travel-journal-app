...
export const checkPremiumStatus = (user: any): boolean => {
...
export const canCreateTravelList = async (
  userId: string
...
export const canAddDestination = async (
  listId: string,
  userId: string
...
export const canCreateJournalEntry = async (
  userId: string
): Promise<{ canCreate: boolean; currentCount: number; limit: number }> => {
  try {
    const user = await UserModel.findById(userId);
...
export const canAddImageToJournal = async (
  userId: string,
