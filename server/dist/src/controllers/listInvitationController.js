...
export const createInvitation = async (req, res) => {
    try {
        const { list, invitee, inviteeEmail, permissionLevel, expiresAt } = req.body;
...
export const getInvitationsByInvitee = async (req, res) => {
    try {
        const { inviteeId } = req.params;
        const { status } = req.query;
        const invitations = await listInvitationService.getInvitationsByInvitee(inviteeId, status);
        res.status(200).json({
            success: true,
            data: invitations,
        });
...
export const getInvitationsByInviter = async (req, res) => {
    try {
        const { inviterId } = req.params;
        const { status } = req.query;
        const invitations = await listInvitationService.getInvitationsByInviter(inviterId, status);
        res.status(200).json({
            success: true,
            data: invitations,
        });
