...
export const updateJournalEntry = async (entryId, updateData, userId) => {
    const journalEntry = await JournalEntryModel.findById(entryId);
    if (!journalEntry) {
        throw new Error("Journal entry not found");
    }
    ...
    const updatedEntry = await JournalEntryModel.findByIdAndUpdate(entryId, { $set: updateData }, { new: true, runValidators: true })
        .populate({
        path: "author",
        select: "fullName username profileImage",
    })
...
export const getJournalEntryStats = async (userId) => {
